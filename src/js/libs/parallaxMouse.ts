...
Например:
	...
	left: -15%;
*/
interface IDedaultConfig {
  init: boolean,
  logging: boolean,
}

export class MousePRLX {
  config: IDedaultConfig

  constructor(props: any, data: any | null = null) {
    const defaultConfig: IDedaultConfig = {
      init: true,
      logging: true,
    }
    ...
    if (this.config.init) {
      const paralaxMouse = document.querySelectorAll<HTMLElement>('[data-prlx-mouse]')
  ...
  paralaxMouseInit(paralaxMouse: any) {
    paralaxMouse.forEach((el: HTMLElement) => {
